...
> ⚠️ 스케줄·워크플로 수치는 `.github/workflows/*.yml` 이 정본. (2026-07-13 감사 반영)
...
│   ├── generate_slack_message.py      # Daily Slack reminder generator (7 AM/9:40 PM KST)
...
│   ├── 2077/                          # 63 additional theme directories (auto-generated)
│   ├── algolia/                       # All contain identical 5 SVG files
│   ├── ... (60+ more theme folders)   # Generated by vn7n24fzkq action
│   └── README.md                      # Theme documentation
│
├── .github/workflows/
│   ├── profile-summary-cards.yml     # Every 6 hours (cron: 0 15,21,3,9 * * *) = 00/06/12/18 KST: GitHub profile cards
│   ├── update_dashboard.yml          # Multi-schedule: reminders (7 AM/9:40 PM KST) + weekly reset (Mon 7 AM KST)
...
**Trigger**: Every 6 hours (`0 15,21,3,9 * * *`) = 00/06/12/18 KST + manual dispatch
**Runs on**: ubuntu-latest
**Permissions**: contents: write

**Process Flow**:
```
1. GitHub Actions runs every 6 hours (00/06/12/18 KST)
   ↓
2. vn7n24fzkq/github-profile-summary-cards@release
   - Fetches accurate GitHub contribution data
   - Generates 5 standard SVG cards × 64 themes = 320 SVG files
...
- **Single-request fetching with disk cache**: 1 GraphQL request total per call.
  ...
  - Effect: profile-summary-cards.yml (every 6 hours) now uses ~1 query/run instead of 7.
...
**Triggers**:
- **Monday 7 AM KST**: `cron: '0 22 * * 0'` (10 PM UTC Sunday) → Weekly reset only
- **Tue-Sun 7 AM KST**: `cron: '0 22 * * 1,2,3,4,5,6'` (10 PM UTC Mon-Sat) → Morning reminder
- **Every day 9:40 PM KST**: `cron: '40 12 * * *'` (12:40 PM UTC) → Evening reminder
...
5. If reminder needed (send_reminder=0):
   a. Detect time:
      ...
      - CURRENT_HOUR=12 (UTC) → 9:40 PM KST → MESSAGE_TYPE="evening"
...
**Trigger**: Push event with path filter `dashboard/data.json` (or manual `workflow_dispatch`)
...
> **⚠️ 이 문서는 upstream(`Piesson/Piesson`) 미러 관점입니다.** vault 모노레포에서 작업할 때는 루트 `CLAUDE.md` 규칙대로 vault에 커밋한 뒤 `scripts/subtree-deploy.sh apps/piesson piesson-upstream` 으로 배포하세요. 아래 `git push` 지시를 vault에서 그대로 따르지 마세요.

**Problem**: GitHub Actions automatically commits every 6 hours (profile cards) and on data.json changes (dashboard). This causes frequent remote-local divergence.
...
- `dashboard/progress_sparklines.svg` - Generated by generate_progress_chart.py
- `profile-summary-card-output/**/*.svg` - Generated by GitHub Actions
- `README.md` auto-generated sections (timestamps, charts, history table) - Updated by generate_svg.py + update_readme_*.py
- Always accept remote version (`--theirs`) in conflicts for these files

**⚠️ README.md conflict 해결 시 절대 주의**:
- `git checkout --ours README.md` 금지 — GitHub Actions가 자동 갱신하는 4개 타임스탬프, 차트 URL, 히스토리 테이블이 모두 되돌려짐
- 올바른 방법: 섹션별로 수동 merge하거나, `--theirs`로 가져온 후 우리 변경만 재적용
- 실수 시 복구: `gh workflow run update_dashboard.yml --repo Piesson/Piesson && gh workflow run profile-summary-cards.yml --repo Piesson/Piesson`
...
- **Cache file**: `dashboard/.stats_cache.json` persists past-year stats across
  runs to reduce API load. Stored at version 1; corrupt or stale cache
...
- [ ] Confirm Slack reminders sent at 7 AM and 9:40 PM KST
- [ ] Test Slack input → confirmation flow

### Monthly
- [ ] Review `weeklyHistory` data structure (currently unused)
- [ ] Check API rate limits (shouldn't be an issue with 6-hourly runs)
