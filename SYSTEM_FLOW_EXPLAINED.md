...
A: **같은 데이터 소스 (GitHub GraphQL API)를 사용하지만, 조회 기간이 다릅니다**:

```
프로필 카드 커밋 (1,476)
├─ 소스: GitHub GraphQL API (graphql_stats.py)
├─ 기간: 2020년 ~ 2025년 전체 (6년간)
│   ├─ 2020-01-01 ~ 2020-12-31 (1년차)
│   ├─ 2021-01-01 ~ 2021-12-31 (2년차)
│   ├─ 2022-01-01 ~ 2022-12-31 (3년차)
│   ├─ 2023-01-01 ~ 2023-12-31 (4년차)
│   ├─ 2024-01-01 ~ 2024-12-31 (5년차)
│   └─ 2025-01-01 ~ 2025-12-31 (6년차, 진행중)
├─ 계산: 각 연도별 쿼리 → 6년간 총합 (누적)
├─ 범위: 모든 레포지토리 (private 포함)
├─ Join date: 2025년 1월 1일 (하드코딩)
├─ Daily avg: 전체 커밋 수 ÷ (오늘 - 2025-01-01) 일수
└─ 업데이트: 매시간 자동 (GitHub Actions)

주간 대시보드 커밋 (120)
├─ 소스: GitHub GraphQL API (get_weekly_commits.py)
├─ 기간: 이번 주만 (월요일 00:00 ~ 현재)
│   └─ 2025-10-27 00:00:00 KST ~ 지금 (2일간)
├─ 계산: 이번 주 월요일부터 현재까지만
├─ 범위: 모든 레포지토리 (private 포함)
└─ 업데이트: data.json 변경 시마다 (Slack 입력 후)

💡 핵심 차이점:
   1. 조회 기간: 프로필(6년 전체) vs 대시보드(이번 주만)
   2. Daily avg 계산: 전체 커밋을 2025-01-01부터 나눔
      → 실제로는 6년 커밋이지만, 2025년부터 계산하므로 높게 표시됨
```
